class Student {
  ...
  factory Student.fromJson(Map<String, dynamic> json) {
    ...
    return Student(
      ...
      hemisLogin: (json['login'] ?? json['hemis_login'] ?? '').toString(),
      ...
      missedHours: json['missed_hours'] is int ? json['missed_hours'] : int.tryParse(json['missed_hours']?.toString() ?? '') ?? 0,
      role: (json['role_code'] ?? json['role'] ?? json['user_role'])?.toString(),
      isPremium: json['is_premium'] == true || json['is_premium'] == 1 || json['is_premium'] == "true",
      balance: json['balance'] is int ? json['balance'] : int.tryParse(json['balance']?.toString() ?? '') ?? 0,
      trialUsed: json['trial_used'] == true || json['trial_used'] == 1,
      premiumExpiry: json['premium_expiry']?.toString(),
      customBadge: json['custom_badge']?.toString(),
      staffRole: (json['staff_role'] ?? json['staff_role_code'] ?? json['role_code'] ?? json['role'])?.toString(),
      facultyId: json['faculty_id'] is int ? json['faculty_id'] : int.tryParse(json['faculty_id']?.toString() ?? ""),
      universityId: json['university_id'] is int ? json['university_id'] : int.tryParse(json['university_id']?.toString() ?? ""),
      firstName: firstName != null ? sentenceCase(firstName.trim()) : null,
      lastName: lastName != null ? sentenceCase(lastName.trim()) : null,
      accommodationName: json['accommodation_name']?.toString(),
    );
  }
